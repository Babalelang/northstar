from sqlalchemy import Column, DateTime, Enum, Date, BigInteger, Boolean, Integer, String, Float, ForeignKey
...
class Player(Base):
    ...
    # Goalkeeper-specific (nullable — only meaningful when playing_position == GK)
    saves = Column(Integer, nullable=True, default=0)
    clean_sheets = Column(Integer, nullable=True, default=0)
    goals_conceded = Column(Integer, nullable=True, default=0)

    # Defender-specific (nullable — only meaningful for CB/LB/RB, but harmless if set for others)
    tackles = Column(Integer, nullable=True, default=0)
    interceptions = Column(Integer, nullable=True, default=0)
    clearances = Column(Integer, nullable=True, default=0)
